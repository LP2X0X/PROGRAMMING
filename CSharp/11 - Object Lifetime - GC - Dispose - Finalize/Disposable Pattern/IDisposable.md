...
In .NET, **"resource"** means anything your program uses that isn't plain managed memory on the heap. The distinction between managed and unmanaged resources determines how you clean them up.

### Unmanaged Resources

An **unmanaged resource** is anything allocated **outside** the CLR's managed heap — something the garbage collector cannot see, track, or free. These are typically OS-level resources obtained through P/Invoke or COM interop:

- **File handles** — `CreateFile`, `FileStream` wraps one internally
- **Network sockets** — TCP connections, HTTP connections
- **Database connections** — the actual connection to the DB server
- **Unmanaged memory** — `Marshal.AllocHGlobal`, `Marshal.AllocCoTaskMem`, native `malloc`
- **OS handles** — window handles (`HWND`), registry keys, mutexes, semaphores
- **GPU / hardware resources** — Direct3D surfaces, device contexts
- **GDI objects** — pens, brushes, fonts in GDI+

The GC has **zero visibility** into these. If you allocate 500 MB of unmanaged memory with `Marshal.AllocHGlobal`, the GC doesn't even know it happened — its memory counters don't change. If you open a file handle, the GC can't close it. **You** must release these explicitly, or they leak until the process exits.

### Managed Resources

A **managed resource** is a managed object (living on the GC heap) that **wraps** an unmanaged resource and implements `IDisposable` to clean it up. The object itself is managed — the GC tracks it and will eventually collect it — but it *holds onto* something unmanaged inside.

Examples:
- `FileStream` — managed object that wraps an OS file handle
- `SqlConnection` — managed object that wraps a native database connection
- `HttpClient` — managed object that wraps socket handles
- `Bitmap` — managed object that wraps a GDI+ image handle

When people say "dispose your managed resources," they mean: **call `Dispose()` on the `IDisposable` objects your class holds**, so those objects can release their internal unmanaged resources.
...
Your class
  │
  ├── holds a SqlConnection         ← managed resource (IDisposable object)
  │     └── wraps a native DB handle    ← unmanaged resource (OS-level)
  │
  ├── holds a FileStream            ← managed resource
  │     └── wraps an OS file handle     ← unmanaged resource
  │
  └── holds an IntPtr (AllocHGlobal) ← directly holding an unmanaged resource
```

- **Most classes** only hold managed resources (other `IDisposable` objects). You call `Dispose()` on them and they handle the unmanaged cleanup internally.
- **Few classes** directly hold unmanaged resources (raw `IntPtr`, handles from P/Invoke). These need the full dispose pattern with a finalizer as a safety net.

### Why the Distinction Matters for Dispose

The `Dispose(bool disposing)` pattern exists specifically because of this distinction:

| `disposing` value | What to clean up | Why |
|---|---|---|
| `true` (called from `Dispose()`) | Both managed and unmanaged resources | Everything is still alive — safe to touch managed objects |
| `false` (called from finalizer) | **Only** unmanaged resources | Other managed objects may already be finalized — touching them is unsafe |

This is covered in detail in Scenario 2 below.
...
See [[Reusing the using Keyword]] for how the `using` statement and `using` declaration guarantee `Dispose()` is called.
