...
```ad-note
If you’ve ever wondered why **instance methods and properties** are inherited this way, here’s the reasoning:  
...
```ad-note
If you’ve ever wondered why the **constructor chain** is used for **static methods and properties**, here’s the reasoning:  
A class in JavaScript is essentially a constructor function, and since functions are also objects, they have their own `__proto__`. Static members belong to the class itself rather than its instances, so they’re stored on the class’s `__proto__`. This allows static methods and properties to be inherited by subclasses and shared across the class hierarchy.
```
